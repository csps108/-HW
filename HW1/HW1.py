...
warp_im2=cv2.warpAffine(cv_im2,M12,(cols1*3, rows1*2),borderMode=cv2.BORDER_TRANSPARENT)
warp_im3=cv2.warpAffine(cv_im3,M13,(cols1*3, rows1*2),borderMode=cv2.BORDER_TRANSPARENT)

temp= np.zeros([rows1*2, cols1*3, 3], np.uint8)
temp1= np.zeros([rows1*2, cols1*3, 3], np.uint8)

res= np.zeros([rows1*2, cols1*3, 3], np.uint8)
for row in range(0, rows1*2):
    for col in range(0, cols1*3):
        if warp_im2[row, col].any():
            temp[row, col] = warp_im2[row, col]
        elif warp_im3[row, col].any():
            temp[row, col] = warp_im3[row, col]
        elif (warp_im3[row, col].any() and warp_im2[row, col].any()):
            if not(warp_im3[row, col].all() and warp_im2[row, col].all()):
                temp[row,col]=bilinear(row,col)
        
            #=========================================#
for row in range(0, rows1):
    for col in range(0, cols1):
        if not warp_im2[row, col].any():
            res[row, col] = cv_im1[row, col]
